# Créditos - Susurros de la Mina

Este documento registra todos los assets de terceros utilizados en el proyecto.

---

## Assets de Texturas

### Kenney Prototype Textures
- **Asset**: Kenney Prototype Textures
- **Autor**: Kenney (https://kenney.nl)
- **Licencia**: CC0 1.0 Universal (Dominio Público)
- **URL**: https://kenney.nl/assets/prototype-textures
- **Uso**: Texturas de prototipo para testing y desarrollo
- **Archivos**: 78 texturas de colores y patrones
- **Fecha de integración**: 25 de Junio, 2026

---

## Assets 3D (Pendientes de Descarga)

### Free Low Poly Mining Assets
- **Asset**: Free Low Poly Mining Assets
- **Autor**: purepoly
- **Licencia**: CC Attribution 4.0
- **URL**: https://sketchfab.com/3d-models/free-low-poly-mining-assets-fb8c435dc8644a1ea4555f59a17313fc
- **Uso**: Herramientas de minería, minerales, props
- **Contenido**: 16 assets únicos (picos, gemas, barriles, etc.)
- **Fecha de integración**: Pendiente

### Modular Caves
- **Asset**: Modular Caves
- **Autor**: ToxaGrom (@3dAssatsForGames)
- **Licencia**: CC Attribution 4.0
- **URL**: https://sketchfab.com/3d-models/modular-caves-66d33b65d4fc471db78a7e4da2232623
- **Uso**: Estructuras modulares de cueva
- **Contenido**: 13 módulos diferentes
- **Fecha de integración**: Pendiente

### Crystal Pack
- **Asset**: Crystal Pack
- **Autor**: iPoly3D
- **Licencia**: CC0 (Dominio Público)
- **URL**: https://poly.pizza/bundles/Crystal Pack
- **Uso**: Cristales y minerales decorativos
- **Contenido**: 28 modelos de cristales
- **Fecha de integración**: Pendiente

---

## Assets de Audio (Pendientes de Descarga)

### Kenney Audio
- **Asset**: Varios packs de audio
- **Autor**: Kenney (https://kenney.nl)
- **Licencia**: CC0 1.0 Universal (Dominio Público)
- **URL**: https://kenney.nl/assets?q=audio
- **Uso**: Efectos de sonido generales, UI, ambiente
- **Fecha de integración**: Pendiente

### OpenGameArt Audio
- **Asset**: CC0 Sound Effects
- **Autor**: Varios artistas
- **Licencia**: CC0 / CC-BY (varía por asset)
- **URL**: https://opengameart.org/content/cc0-sound-effects
- **Uso**: Efectos de sonido de minería, ambiente
- **Fecha de integración**: Pendiente

---

## Herramientas Utilizadas

### Motor de Desarrollo
- **Godot Engine 4.7** - Motor de videojuegos open source
- **Licencia**: MIT License

### Herramientas de Código
- **GDScript** - Lenguaje de script de Godot
- **VS Code** - Editor de código (recomendado)

### Herramientas de Arte (Futuras)
- **Blender** - Modelado 3D (https://blender.org)
- **GIMP** - Edición de texturas (https://gimp.org)
- **Audacity** - Edición de audio (https://audacityteam.org)

---

## Licencias

### CC0 1.0 Universal (Dominio Público)
Permite usar, modificar y distribuir sin atribución requerida.
Ideal para assets que no requieren crédito.

### CC Attribution 4.0
Permite usar, modificar y distribuir con atribución requerida.
Debe incluirse crédito al autor original.

### MIT License
Permite uso libre con atribución minimal.
Usado por Godot Engine.

---

## Notas

- Todos los assets de terceros deben verificarse antes de integrar
- Preferir siempre licencias CC0 cuando sea posible
- Para licencias CC-BY, incluir crédito en este archivo y en los créditos del juego
- Documentar cualquier asset creado específicamente para este proyecto

---

*Última actualización: 25 de Junio, 2026*